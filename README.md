...
`data/contreras_af3/run_pipeline_all.sh` expect the
[`SolNRCH_foldome`](https://github.com/amiralito/SolNRCH_foldome)
repository cloned as a sibling of `sni_ml`:

    sni_paper/
    ├── sni_ml/                 # this repository
    └── SolNRCH_foldome/        # git clone https://github.com/amiralito/SolNRCH_foldome

`SolNRCH_foldome` defines the canonical 11 SNI features and ships the
`resistosome_pipeline`. We do not vendor it here.
...
./run_pipeline_all.sh   # calls ../SolNRCH_foldome/resistosome_pipeline
```
